...
## 9. Claude の Routine で原稿を書く（任意）

Gemini Spark の代わりに、Claude の Routine（claude.ai/code の定期実行）でニュースを調べて、シートに行を追加する。書き込みには `cmd/queue-add` を使う。

書き込み専用のサービスアカウントを作る。GCP のロールは何も付けない。

```powershell
$WRITER = "sheet-writer@${PROJECT_ID}.iam.gserviceaccount.com"
gcloud iam service-accounts create sheet-writer "--display-name=News queue writer"
echo $WRITER
```

スプレッドシート `daily_news_queue` の「共有」で、表示されたアドレスを **編集者** として追加する。

鍵を作り、base64 にしてクリップボードに入れたら、鍵のファイルはすぐに消す。

```powershell
$KEYFILE = "$env:TEMP\sheet-writer-key.json"
gcloud iam service-accounts keys create $KEYFILE "--iam-account=$WRITER"
[Convert]::ToBase64String([IO.File]::ReadAllBytes($KEYFILE)) | Set-Clipboard
Remove-Item $KEYFILE
```

Claude のクラウド環境の設定（セッションのタイトルバーの環境メニュー → Edit）で、環境変数に `SHEET_WRITER_KEY=<貼り付け>` を追加する。鍵をリポジトリやチャットに貼らない。設定は、次に始めたセッションから使われる。

鍵が漏れたときは、鍵を無効にする（このアカウントが触れるのはこのスプレッドシートだけ）。

```powershell
gcloud iam service-accounts keys list "--iam-account=$WRITER"
...
`cmd/queue-add` は `SPREADSHEET_ID`、`SHEET_NAME` と、`SHEET_WRITER_KEY`（サービスアカウントの JSON 鍵。そのままか base64）を読む。
