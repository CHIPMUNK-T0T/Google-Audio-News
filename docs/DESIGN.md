...
| タブ名 | `daily_news_queue`（**要確認**。違う名前なら、タブ名を変えるか、ジョブの `SHEET_NAME` をそれに合わせる） |
...
| B | `created_at` | Spark | `YYYY-MM-DD HH:MM:SS`（日本時間）。YouTube の説明欄に表示する |
| C | `title` | Spark | 100文字以内。`<` と `>` は使わない（超えた分や記号はジョブ側で切り詰め・置き換えをする） |
...
・created_at は YYYY-MM-DD HH:MM:SS（日本時間）。
・title は100文字以内で、< と > を使わない。
...
3. **YouTube の制約**: タイトルは100文字まで、説明欄は5,000バイトまで。どちらも `<` と `>` は使えない。コードで全角に置き換え、長さを切り詰めている。
...
- シートのタブ名が `daily_news_queue` かどうか（4章）。
